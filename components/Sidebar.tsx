...
const TrophyIcon: React.FC<{ rank: number }> = ({ rank }) => {
    const colors = ['text-[#D4AF37]', 'text-[#C0C0C0]', 'text-[#CD7F32]']; // Gold, Silver, Bronze
    if (rank > 2) return <span className="w-6 text-center text-[#F5F1E9] opacity-60 font-medium">{rank + 1}</span>;
    return (
...
const Sidebar: React.FC<SidebarProps> = ({ players, currentUser, onNewGame }) => {
  ...
  return (
    <aside className="w-full md:w-80 bg-[#0D1B1E] text-[#F5F1E9] p-4 md:p-6 flex flex-col shadow-2xl md:h-full">
      <div className="mb-6">
        <h1 className="text-4xl font-extrabold tracking-tighter">Pin the Dot</h1>
        <p className="text-[#F5F1E9] opacity-70 font-medium">Leaderboard</p>
      </div>
          ...
          return (
            <div
              ...
              className={`flex items-center p-3 rounded-lg transition-all duration-300 ${
                isCurrentUser
                  ? 'bg-[#B55B3E] shadow-lg scale-105'
                  : 'bg-[#1A2B2E] hover:bg-[#2A4747]'
              }`}
            >
              <div className="w-8 flex-shrink-0 flex justify-center items-center">
                <TrophyIcon rank={index} />
              </div>
              <p className="flex-1 font-semibold text-[#F5F1E9] truncate ml-3">{player.name}</p>
              <p className="font-bold text-lg text-[#F5F1E9]">{player.score}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="mt-6">
        <button
          onClick={onNewGame}
          className="w-full bg-[#B55B3E] hover:bg-[#a14f34] text-white font-bold py-3 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#0D1B1E] focus:ring-[#B55B3E] transition-all duration-300 transform hover:scale-105 shadow-xl"
        >
          New Game
        </button>
      </div>
